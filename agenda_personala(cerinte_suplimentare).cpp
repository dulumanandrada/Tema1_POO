...
class Cumparatura
{
    ...
    friend Cumparatura operator-(Cumparatura&, Cumparatura&);
    Cumparatura operator-();
    Cumparatura operator--();
    Cumparatura operator--(int x);
    operator int() const
    {
        return cantitate;
    }
    ...
    Cumparatura(string s, double c, double p)
    {
        nume = s;
        cantitate = c;
        pret = p;
    ...
    //constructor parametrizat cu tip bool
    Cumparatura(bool ok)
    {
        if(ok == true) this->citire();
        else this->citire_fisier();
    }
...
void Cumparatura::citire_fisier()
{
    fin >> nume >> cantitate >> pret;
}
...
//schimba semnul (doar de la cantitate, asa ar parea logic)
Cumparatura Cumparatura::operator-()
{
...
Cumparatura Cumparatura::operator--(int x)
{
    cantitate--;
    return *this;
}
...
class ShoppingList
{
    ...
    void citire();
    void afisare();
    friend istream& operator>>(istream&, ShoppingList&);
    friend ostream& operator<<(ostream&, ShoppingList&);
...
void ShoppingList::citire()
{
    ...
    for(int i=1; i<=NR; i++)
        {
            cin >> L[i];
            TOTAL = TOTAL + (L[i].get_cantitate() * L[i].get_pret());
        }
}
...
istream& operator>>(istream& i, ShoppingList& l)
{
    l.TOTAL = 0;
    i >> l.NR;
    ...
    {
        cin >> l.L[i];
        l.TOTAL = l.TOTAL + (l.L[i].get_cantitate() * l.L[i].get_pret());
    }

    return i;
}

ostream& operator<<(ostream& o, ShoppingList& l)
{
    for(int i = 1; i <= l.NR; i ++)
    {
        cout << l.L[i] <<"\n";
    }
    cout << "Total de: " << l.TOTAL;
    return o;
}

ShoppingList ShoppingList::operator=(ShoppingList& l)
{
    NR = l.NR;
    for(int i = 1; i <= NR; i++)
        L[i] = l.L[i];
...
class Individ
{
    ...
    //am schimbat aceasta fnct din friend in fnct membra
    bool operator==(Individ&);
...
{
    return(nume == i.nume && prenume == i.prenume);
}
...
int main()
{
    ...
    while(task)
    {
        ...
        if(task == 10)
        {
            //Cumparatura z = Cumparatura("oua", 2, 3);
            //cout << z;

            //Cumparatura z = Cumparatura(true);
            //cout << z <<"\n";
            //Cumparatura q = Cumparatura(false);
            //cout << q <<"\n";
